...
async function main() {
  ...
  try {
    ...
    if (admins > 0) {
      console.log('[bootstrap-admin] Ya existe un ADMIN. Nada que hacer.');
      return;
    }
    ...
    if (!email || !password) {
      console.warn(
        '[bootstrap-admin] No hay ADMIN y faltan INITIAL_ADMIN_EMAIL/INITIAL_ADMIN_PASSWORD.',
      );
      return;
    }
    if (password.length < 12 || WEAK.has(password.toLowerCase())) {
      throw new Error(
        'INITIAL_ADMIN_PASSWORD debe tener al menos 12 caracteres y no ser común.',
      );
    }
    if (await prisma.user.findUnique({ where: { email } })) {
      throw new Error(
        'Ya existe un usuario con ese email y no es ADMIN. No se modifica.',
      );
    }
