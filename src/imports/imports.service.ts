...
export class ImportsService {
  ...
  async processMovementsCSV(
    ...
    if (rows.length > MAX_IMPORT_ROWS) {
      throw new BadRequestException(
        `El archivo excede el límite de ${MAX_IMPORT_ROWS} filas para esta fase.`,
      );
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!sku || !warehouseCode || !typeStr || !quantityStr) {
        result.errors.push({
          ...
          sku,
          reason:
            'Faltan campos obligatorios (sku, warehouseCode, type, quantity)',
        });
        result.errorCount++;
        continue;
      }
      ...
      if (!(typeStr in MovementType)) {
        result.errors.push({
          rowNumber,
          sku,
          reason: `Tipo de movimiento inválido: ${typeStr}. Tipos permitidos: INBOUND, OUTBOUND, TRANSFER, ADJUSTMENT`,
        });
        result.errorCount++;
        continue;
      }
      ...
      if (isNaN(quantity) || quantity <= 0) {
        result.errors.push({
          rowNumber,
          sku,
          reason: 'La cantidad debe ser un número mayor a 0',
        });
        result.errorCount++;
        continue;
      }
      ...
      if (!productId) {
        result.errors.push({
          rowNumber,
          sku,
          reason: `SKU no encontrado en el sistema: ${sku}`,
        });
        result.errorCount++;
        continue;
      }
      ...
      if (!warehouseId) {
        result.errors.push({
          rowNumber,
          sku,
          reason: `Código de depósito no encontrado: ${warehouseCode}`,
        });
        result.errorCount++;
        continue;
      }
      ...
      try {
        ...
        result.errors.push({
          ...
          sku,
          reason:
            error.message || 'Error procesando el movimiento en base de datos',
        });
        result.errorCount++;
      }
    }
  ...
  async processProducts(file: Express.Multer.File): Promise<ImportResultDto> {
    ...
    if (rows.length > MAX_IMPORT_ROWS)
      throw new BadRequestException(
        `El archivo excede el límite de ${MAX_IMPORT_ROWS} filas.`,
      );
